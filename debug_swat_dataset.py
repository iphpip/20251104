...
import numpy as np
import yaml
from data.datasets import SWaTDataset  # 假设已实现SWaT数据集类

def check_swat_ratio(config):
    """检查SWaT数据集的异常比例配置合理性"""
    dataset_name = "SWaT"
    ...
    print(f"配置的训练集异常比例：{dataset_config['train_anomaly_ratio']:.4f}（通常应为0，纯正常数据）")
    print(f"配置的测试集异常比例：{dataset_config['test_anomaly_ratio']:.4f}\n")

    try:
        # 加载训练集（SWaT训练集通常无异常）
        train_set = SWaTDataset(
            data_path=data_path,
        ...
        # 对比配置与实际（SWaT训练集需严格为0，测试集允许±0.01误差）
        train_valid = (train_ratio == 0.0)
        test_diff = abs(test_ratio - dataset_config['test_anomaly_ratio'])
        test_valid = (test_diff <= 0.01)

        if train_valid and test_valid:
            print(f"✅ {dataset_name} 配置合理（训练集无异常，测试集差异在允许范围内）")
        else:
            print(f"❌ {dataset_name} 配置不合理！建议调整：")
            if not train_valid:
                print(f"SWaT训练集应为纯正常数据，但实际异常比例为 {train_ratio:.4f}（可能数据加载错误）")
            if not test_valid:
